(function() {
	...
	var Mdate = function(el, opts) {
		...
		this.format = opts.format || dateopts.format;this.dateBoxShow()
	};
	Mdate.prototype = {
		constructor : Mdate,
		...
		createDateBox : function() {
			...
			if (!MdatePlugin) {
				...
				dateContentBox.id = "MdatePlugin";d.body.appendChild(dateContentBox);
				MdatePlugin = d.getElementById("MdatePlugin")
			}
			MdatePlugin.setAttribute("class", "slideIn");that.createDateUi();
			var yearUl = d.getElementById("yearUl");
			var monthUl = d.getElementById("monthUl");
			var dayUl = d.getElementById("dayUl");
			yearUl.innerHTML = that.createDateYMD("year");that.initScroll();that.refreshScroll()
		},
		...
		initScroll : function() {
			...
			yearScroll = new iScroll("yearwrapper", {
				...
				onScrollEnd : function() {
					...
					if(that.format.indexOf("mm") > 0 ){
						monthUl.innerHTML = that.createDateYMD("month");monthScroll.refresh();try {
			...
			monthScroll = new iScroll("monthwrapper", {
				...
				onScrollEnd : function() {
					...
					if (indexM == 1 && yearTag != that.beginYear) {
						monthTag = 1
					} else if(that.format.indexOf("mm") > 0 ) {
						monthTag = monthUl.getElementsByTagName("li")[indexM].getAttribute("data-month")
					}
					if(that.format.indexOf("dd") > 0 ){
						dayUl.innerHTML = that.createDateYMD("day");dayScroll.refresh();try {
			...
			dayScroll = new iScroll("daywrapper", {
				...
				onScrollEnd : function() {
					...
					if (indexD == 1 && monthTag != that.beginMonth) {
						dayTag = 1
					} else if(that.format.indexOf("dd") > 0 ){
						dayTag = dayUl.getElementsByTagName("li")[indexD].getAttribute("data-day")
					}
				}
			})
		...
		refreshScroll : function() {
			...
			inputMonth -= initM;
			inputDay -= initD;yearScroll.refresh();yearScroll.scrollTo(0, (new Date().getFullYear()-that.beginYear) * 40, 300, true);monthScroll.scrollTo(0, inputMonth * 40, 300, true);dayScroll.scrollTo(0, inputDay * 40, 300, true)
		},
		dateSure : function() {
			var that = this;
			...
			sureBtn.onclick = function() {
				monthTag=monthTag<10?'0'+monthTag:monthTag;
				dayTag=dayTag<10?'0'+dayTag:dayTag
				if (that.format == "yyyy") {
					that.acceptId.value = yearTag 
				} else if (that.format == "yyyymm") {
		...
		dateCancel : function() {
			MdatePlugin.setAttribute("class", "slideOut");setTimeout(function() {
