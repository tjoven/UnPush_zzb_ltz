...
import com.async.future.SimpleCancellable;
import com.async.http.libcore.Charsets;
import com.async.http.libcore.DiskLruCache;
import com.async.http.libcore.RawHeaders;
import com.async.http.libcore.ResponseHeaders;
import com.async.http.libcore.ResponseSource;
import com.async.http.libcore.StrictLineReader;
...
public class ResponseCacheMiddleware extends SimpleMiddleware {
    ...
    private class CachedSocket extends DataEmitterBase implements AsyncSocket {
        ...
        void spewInternal() {
            if (pending.remaining() > 0) {
                com.async.Util.emitAllData(CachedSocket.this, pending);
                if (pending.remaining() > 0)
                    return;
            }
    ...
    private static class BodyCacher extends FilteredDataEmitter implements Parcelable {
        ...
        public void onDataAvailable(DataEmitter emitter, ByteBufferList bb) {
            if (cached != null) {
                com.async.Util.emitAllData(this, cached);
    ...
    private static class BodySpewer extends FilteredDataEmitter {
        ...
        void spewInternal() {
            if (pending.remaining() > 0) {
                com.async.Util.emitAllData(BodySpewer.this, pending);
                if (pending.remaining() > 0)
                    return;
            }
            ...
            try {
                ...
                din.readFully(buffer.array(), buffer.arrayOffset(), (int)contentLength);
                pending.add(buffer);
                com.async.Util.emitAllData(this, pending);
                assert din.read() == -1;
                allowEnd = true;
                report(null);
            }
    ...
    public void onBodyDecoder(OnBodyData data) {
        CachedSocket cached = (CachedSocket) com.async.Util.getWrappedSocket(data.socket, CachedSocket.class);
        if (cached != null) {
