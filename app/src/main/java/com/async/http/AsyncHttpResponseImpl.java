...
import com.async.*;
import com.async.LineEmitter.StringCallback;
import com.async.callback.CompletedCallback;
import com.async.callback.WritableCallback;
import com.async.http.filter.ChunkedOutputFilter;
...
abstract class AsyncHttpResponseImpl extends FilteredDataEmitter implements AsyncHttpResponse {
    ...
    void setSocket(AsyncSocket exchange) {
        ...
        com.async.Util.writeAll(exchange, rs.getBytes(), new CompletedCallback() {
            @Override
            public void onCompleted(Exception ex) {
                if (mWriter != null) {
