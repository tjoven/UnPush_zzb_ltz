...
import com.async.future.DependentCancellable;
...
class SocketIOConnection {
    ...
    public void disconnect(SocketIOClient client) {
        ...
        ImCoreService.mConnected=false;
    ...
    void reconnect(final DependentCancellable child) {
    	//unigo.utility.Log.write(1, "reconnect", "reconnect:"+isConnected()+";ImCoreService.mConnected:"+ImCoreService.mConnected,"");
        if (isConnected()) {
            return;
        }
        if(ImCoreService.mConnected){
        	return;
        }
        ...
        Cancellable connecting = httpClient.executeString(request).then(new TransformFuture<WebSocket, String>() {
            @Override
            protected void transform(String result) throws Exception {
                ...
                final String sessionUrl = uri + "websocket/" + session  + query;

                httpClient.websocket(sessionUrl, null, null).setCallback(getCompletionCallback());
            }
        ...
        Runnable heartbeatRunner = new Runnable() {
            ...
            public void run() {
                ...
                if (heartbeat <= 0 || ws != webSocket || ws == null || !ws.isOpen())
                    return;
                webSocket.send("2:::");
                unigo.utility.Log.write(1, "setupHeartbeat", "setupHeartbeat:"+heartbeat+"========"+ws.isOpen(),"");
    ...
    private void delayReconnect() {
        ...
        if (!disconnected)
            return;
        if(reconnectDelayflag){
        	return;
        }
        httpClient.getServer().postDelayed(new Runnable() {
            @Override
            public void run() {
            	reconnectDelayflag=true;
                //reconnect(null);
            }
        }, reconnectDelay);
        reconnectDelay = 55000L;
    }

    long reconnectDelay = 10000L;
    boolean reconnectDelayflag=false;
    private void reportDisconnect(final Exception ex) {
        if (ex != null) {
            request.loge("socket.io disconnected", ex);
            unigo.utility.Log.write(1, "delayReconnect", "isConnected:"+reconnectDelay+"ms  error:"+ex.toString(),"");
        }
        else {
...
//                    assert false;
    ...
    private Acknowledge acknowledge(final String _messageId, final String endpoint) {
        ...
        Acknowledge ac=new Acknowledge() {
            @Override
    ...
    private void attach() {
        ...
        webSocket.setStringCallback(new WebSocket.StringCallback() {
            ...
            public void onStringAvailable(String message) {
                try {
                    Log.d("setStringCallback", "Message: " + message);
                    ...
                    switch (code) {
                        ...
                        case 2:
                            // heartbeat
                        	unigo.utility.Log.write(1, "heartbeat", String.format("Got heartbeat: %s", message),"");
                            webSocket.send("2::");
                            break;
                        case 3: {
                            // message
